#!/usr/bin/env python3
"""Law Commission of India static-data builder — orchestrator.

Per scheduled (or workflow_dispatch) run:
  1. Walk the 22 Commission-term pages on https://lawcommissionofindia.nic.in/,
     parse their HTML tables for (report_number, title, date_submitted,
     pdf_url, commission_term).
  2. Save reports.json (primary file).
  3. Extract text from missing PDFs as fits the per-run budget
     (MAX_EXTRACTIONS_PER_RUN, MAX_RUN_SECONDS) — newest reports first
     (highest report number = most recent).
  4. (Derive phase) Build manifest + sharded bundle + sharded index +
     meta + audit.

Output goes under docs/lc/, served at sansadsaar-data.naklitechie.com/lc/.

Independence Principle: no imports from cag/scraper.py, drsc/build_static.py,
or bills/sansad/scraper.py. Small duplication of HTTP / jitter / checkpoint
primitives is the explicit choice per CONV.md.

See plan/law-commission-recon-001.md + plan/adding-a-new-corpus.md for
context. Structurally a near-twin of build_cag.py with two simplifications:
no detail-page fetch phase (per-Commission page IS the detail surface),
and no listing-page-budget knob (we always walk the fixed list of 22 terms;
~5 sec for the whole walk).
"""
...
def load_existing_reports() -> dict[int, dict]:
    """Returns int-keyed dict of LC metadata, keyed by report_number.
    Disk format JSON-keys-as-strings; we parse back to ints for in-memory
    work to keep sort + lookup natural.
    """
...
def save_reports(reports: dict[int, dict]) -> None:
    # Sort by report_number desc (newest first) for stable diffs and
...
# ── Phase 1+2: walk all 22 Commission pages + merge into reports.json ──────
#
# Unlike CAG (which has a paginated listing + a separate detail-page step),
# LC's per-Commission page contains the FULL row metadata in its table —
# no separate detail fetch. So enumerate_ids + fetch_new_metadata collapse
# into one walk_all step. ~5 seconds for all 22 pages.

def walk_and_merge_reports(existing: dict[int, dict]) -> tuple[dict[int, dict], dict]:
    """Walk all 22 Commission pages, build the latest reports dict, and
    merge with `existing` (fresh wins on conflict; old kept on delisting).
    Returns (merged_dict, stats).

    Mirrors the Bills records-merge pattern (see build_bills_sansad.py
    merge_records) — once a report is mirrored we never lose it even if
    upstream removes it. Archival promise.
    """
    print("[Walk] Fetching all 22 Commission-term pages...")
    t0 = time.time()
    fresh: dict[int, dict] = {}
    skipped_norow = 0
    for r in walk_all_commissions():
        if r.report_number in fresh:
            # Same number on two pages — shouldn't happen but defensive.
            print(f"  [walk] duplicate report_number {r.report_number}; keeping first")
            continue
        fresh[r.report_number] = {
            "report_number":      r.report_number,
            "title":               r.title,
            "date_submitted":      r.date_submitted,
            "date_submitted_raw":  r.date_submitted_raw,
            "year":                r.year,
            "commission_term":     r.commission_term,
            "pdf_url":             r.pdf_url,
        }
    print(f"  walked all 22 Commissions in {time.time()-t0:.1f}s — {len(fresh)} reports total")

    # Merge: fresh wins on overlap; existing-only entries kept (archival).
    merged = dict(existing)
    new_count = 0
    updated_count = 0
    for num, rec in fresh.items():
        if num in merged:
            if merged[num] != rec:
                updated_count += 1
            merged[num] = rec
            ...
            new_count += 1
            merged[num] = rec
    kept_legacy = sum(1 for num in merged if num not in fresh)
    stats = {"new": new_count, "updated": updated_count, "kept_legacy": kept_legacy}
    print(f"  merged: {len(merged)} total (new={new_count}, updated={updated_count}, kept_legacy={kept_legacy})")
    return merged, stats
...
def extract_missing_texts(reports: dict[int, dict], *, deadline: float) -> dict:
    ...
    # bundled text is the source of truth after the 2026-05-14 docs/text/
    # cleanup, so checking only `.txt` existence on disk would cause
    # re-extraction loops for already-bundled records.
    bundled_ids: set = set()
    ...
    # Newest first (highest report_number wins — LC numbering is monotonic
    # across all 22 Commission terms).
    ...
    # Calculate remaining budget from the unified deadline so a long walk
    # phase doesn't push the extract past the 60-min GH Actions cap.
    # (See cag-backfill runs 25611721332 / 25612861100 et al. for the
    # original failure mode this pattern prevents.)
    ...
    with ThreadPoolExecutor(max_workers=EXTRACT_WORKERS) as ex:
        ...
        for fut in as_completed(futures):
            ...
            if (extracted_since_checkpoint >= CHECKPOINT_EVERY_N or
                ...
                checkpoint_commit(
                    f"Auto-checkpoint LC primary data (extracted={len(extracted)} this run) [{ts}]",
                    ["docs/lc/reports.json", "docs/lc/text/"],
                )
                extracted_since_checkpoint = 0
    ...
    if extracted_since_checkpoint > 0:
        ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%MZ")
        checkpoint_commit(
            f"Auto-checkpoint LC primary data (final, extracted={len(extracted)} this run) [{ts}]",
            ["docs/lc/reports.json", "docs/lc/text/"],
        )
...
def _load_bundled_ids() -> set[str]:
    ...
    written by write_text_shards). Empty set on missing/unreadable.
    For LC: each composite_id is the report_number string.
    """
    texts_meta_path = DOCS / "texts-meta.json"
...
def build_manifest() -> dict:
    ...
    glob for transient text/<id>.txt files between extract + derive.
    """
...
def phase_extract() -> None:
    """Phase 1 — produce primary files (reports.json + text/<number>.txt).
    ...
    reports.json is deterministic given upstream content + the
    walk_and_merge contract (fresh wins; old kept on delisting).
    """
    overall_deadline = time.monotonic() + MAX_RUN_SECONDS

    print("\n[Extract 1/2] Walking 22 Commission pages + merging into reports.json...")
    existing = load_existing_reports()
    print(f"  existing on disk: {len(existing)} reports")
    merged, walk_stats = walk_and_merge_reports(existing)
    save_reports(merged)
    print(f"  reports.json: {len(merged)} total "
          f"({walk_stats['new']} new, {walk_stats['updated']} updated, "
          f"{walk_stats['kept_legacy']} kept legacy)")

    print("\n[Extract 2/2] Extracting missing texts (priority: highest report_number first)...")
...
def phase_derive() -> None:
    ...
    search-index-*.json, meta.json, audit.json) from the on-disk primary
    state. Pure function of reports.json + text/. Owned by lc-derive.yml;
    one run at a time via `cancel-in-progress: true` on the lc-derive group.
    """
    ...
    # preservation logic carries them forward.
    items = sorted(
        (key, DOCS / entry["url"])
...
def main():
    ...
    BUILD_PHASE=extract — for lc.yml writers. Produces only reports.json +
                          text/<number>.txt; never touches derived files.
                          LC has no separate backfill workflow — the corpus
                          is small enough that 3 daily runs clear the full
                          historical backlog. See
                          plan/law-commission-recon-001.md.
    BUILD_PHASE=derive  — for lc-derive.yml. Reads disk state, regenerates
                          manifest.json + search-bundle-*.json +
                          search-index-*.json + meta.json + audit.json.
                          Single owner of derived files (concurrency
                          cancel-in-progress: true).
    BUILD_PHASE=all     — legacy / local-dev convenience. Runs both phases
                          in one process; don't use in CI.
    """
    ...
    print("=== ParliamentWatch Law Commission static builder ===")
